...
# </p>
# '''
__auto_select__ = "Select Serial Port"
...
class FlashingThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            command.extend(["--baud", "460800",
                        "--after", "hard_reset",
                        "write_flash",
                        "--flash_size", "detect",
                        "--flash_mode", "dio",
                        "0x0000", self._config.firmware_path])

            # if self._config.erase_before_flash:
            #     command.append("--erase-all")

            print("Command: esptool.py --chip esp32 %s\n" % " ".join(command))
...
# ---------------------------------------------------------------------------
class DIYFBFlasher(wx.Frame):

    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, -1, title, size=(725, 300), style=wx.DEFAULT_FRAME_STYLE | wx.NO_FULL_REPAINT_ON_RESIZE)
        ...
        self.Centre(wx.BOTH)
        self.Show(True)
        print("Connect the USB cable to your device")
        print("\nSelect the serial port and firmware file")
        # print("\nBrowse to the firmware installation file")
        # print("\n(Example... V2.0.0_24112601_install.bin)")
        print("\nPress the 'Upload Firmware' button")
        print("\nOnce upload is complete. Restart your device")
    ...
    def _init_ui(self):
        ...
        # add_erase_radio_button(erase_boxsizer, 1, True, "yes", erase is True)

        button = wx.Button(panel, -1, "Upload Firmware")
        ...
        file_label = wx.StaticText(panel, label="Firmware File")
    ...
    def _build_status_bar(self):
        ...
        status_text = "DIYFB Firmware Loader V:%s" % __version__
    ...
    def _build_menu_bar(self):
        ...
        # exit_item = file_menu.Append(wx.ID_EXIT, "E&xit\tCtrl-Q", "Exit DIYFB Firmware Loader")
        ...
        # Help menu
        help_menu = wx.Menu()
        help_item = help_menu.Append(wx.ID_ABOUT, '&About DIYFB Firmware Loader', 'About')
    ...
    def _get_config_file_path():
        return wx.StandardPaths.Get().GetUserConfigDir() + "/DIYFB-Firmware-Loader.json"
...
class MySplashScreen(wx.adv.SplashScreen):
    def __init__(self):
        ...
        self.__fc = wx.CallLater(3000, self._show_main)
    ...
    def _show_main(self):
        frame = DIYFBFlasher(None, "DIYFB Firmware Loader")
        frame.Show()
        if self.__fc.IsRunning():
            self.Raise()
...
class App(wx.App, wx.lib.mixins.inspection.InspectionMixin):
    def OnInit(self):
        ...
        self.SetAppName("DIYFB Firmware Loader")
